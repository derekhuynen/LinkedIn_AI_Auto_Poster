...
describe('Gallery', () => {
	...
	it('opens a modal with the full post when a card is clicked, and closes it', async () => {
		mocks.getPosts.mockResolvedValue(PAGE_1);
		render(<Gallery />);

		const card = await screen.findByRole('button', {
			name: /read post: why idempotency/i,
		});
		await userEvent.click(card);

		// Modal dialog opens with the post content.
		const dialog = await screen.findByRole('dialog');
		expect(dialog).toBeInTheDocument();

		// Closing it removes the dialog.
