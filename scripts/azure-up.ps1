...
# Image generation toggle (gpt-image-1). Defaults off unless the config enables it;
# the demo config enables it.
$enableImage = $false
...
# --allow-blob-public-access true: generated cover images are served to the
# dashboard <img> tags via public blob URLs (they are non-sensitive AI art).
Invoke-Native 'az' @('storage', 'account', 'create', '-n', $storageName, '-g', $rg, '-l', $cfg.location,
	'--sku', 'Standard_LRS', '--kind', 'StorageV2', '--allow-blob-public-access', 'true', '-o', 'none') | Out-Null
...
if (-not $SkipDeploy) {
	...
	if ($seedGallery) {
		...
		try {
			...
			# Seeds from pre-saved examples (web/lib/examples.json + /samples images);
			# no live generation or model access needed.
