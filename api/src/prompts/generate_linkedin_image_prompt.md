...
- **Format**: Landscape orientation preferred for better LinkedIn display
...
Generate an image generation prompt that includes:
...
Return only the image generation prompt, not explanations. The prompt should be specific and descriptive enough for the image model to create a professional, topic-relevant image.
