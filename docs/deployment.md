...
| Azure OpenAI            | A `gpt-5-mini` (text) and `gpt-image-1` (image) deployment. gpt-image-1 is region-limited (e.g. swedencentral); both can live on one AIServices resource there. |
...
| `ENABLE_IMAGE_GENERATION`       | `true`                                               | Toggle image generation (needs gpt-image-1) |
